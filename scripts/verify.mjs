...
assert.match(indexHtml, /id="vault-passphrase"[^>]+minlength="6"/);
assert.match(indexHtml, /id="vault-confirm"[^>]+minlength="6"/);
assert.match(indexHtml, /לפחות 6 תווים\. להגנה חזקה יותר מומלץ לבחור 12 תווים ומעלה\./);
assert.match(appSource, /MIN_PASSPHRASE_LENGTH/);
